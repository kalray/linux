...
void fixedrange_init(void)
{
	...
	/* Allocate the PUD page */
	fixmap_pud_p = (pud_t *) memblock_alloc(PAGE_SIZE, PAGE_SIZE);
	memset(fixmap_pud_p, 0, PAGE_SIZE);
	set_pud(pud, __pud((unsigned long) fixmap_pud_p));

	pmd = pmd_offset(pud, vaddr);
	/* Allocate the PMD page */
	fixmap_pmd_p = (pmd_t *) memblock_alloc(PAGE_SIZE, PAGE_SIZE);
	memset(fixmap_pmd_p, 0, PAGE_SIZE);
	set_pmd(pmd, __pmd((unsigned long) fixmap_pmd_p));
}
