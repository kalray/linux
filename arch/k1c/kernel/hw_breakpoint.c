...
#define L1_LINE_MASK		((u64) K1C_DCACHE_LINE_SIZE - 1)
...
#define hw_breakpoint_remap(idx) (K1C_HW_BREAKPOINT_COUNT - 1 - (idx))

/* Breakpoint currently in use */
static DEFINE_PER_CPU(struct perf_event *, hbp_on_reg[K1C_HW_BREAKPOINT_COUNT]);

/* Watchpoint currently in use */
static DEFINE_PER_CPU(struct perf_event *, hwp_on_reg[K1C_HW_WATCHPOINT_COUNT]);
...
int hw_breakpoint_slots(int type)
{
	switch (type) {
	case TYPE_INST:
		return K1C_HW_BREAKPOINT_COUNT;
	case TYPE_DATA:
		return K1C_HW_WATCHPOINT_COUNT;
	default:
 ...
 * stored
 * Return: Nothing
 * Observation: Because of the k1c watchpoint range problem, both hardware
					...
					struct arch_hw_breakpoint *hw)
{
	...
	if (begin < addr_l1_aligned) {
		hw->wp.hw_wp_addr[0] = begin;
		hw->wp.hw_wp_range[0] = fls64(begin ^ (addr_l1_aligned - 1));

		hw->wp.use_wp1 = 1;
		hw->wp.hw_wp_addr[1] = addr_l1_aligned;
		hw->wp.hw_wp_range[1] = fls64(addr_l1_aligned ^ end);
	} else if (addr_l1_aligned != end_l1_aligned) {
		hw->wp.hw_wp_addr[0] = addr_l1_aligned;
		hw->wp.hw_wp_range[0] =
			fls64(addr_l1_aligned ^ (end_l1_aligned - 1));

		hw->wp.use_wp1 = 1;
		hw->wp.hw_wp_addr[1] = end_l1_aligned;
		hw->wp.hw_wp_range[1] = fls64(end_l1_aligned ^ end);
	} else {
		hw->wp.use_wp1 = 0;
		hw->wp.hw_wp_addr[0] = addr_l1_aligned;
		hw->wp.hw_wp_range[0] = fls64(addr_l1_aligned ^ end);
	}

	if (!hw->wp.use_wp1) {
		hw->wp.hw_wp_addr[1] = 0;
		hw->wp.hw_wp_range[1] = 0;
	}
	hw->wp.hit_info = 0;
}
 ...
 *
 * Observation: Because of the k1c watchpoint range problem, both hardware
 * watchpoints are used for index 0
 */
int ptrace_request_hw_watchpoint(int idx)
{
	int res;

	/* Request the both watchpoints. First W0 */
	res = reserve_one_hw_watchpoint(idx);
	if (res)
		return res;

	/* W0 is ours, request W1 */
...
{
	if (enable) {
		set_hw_wp_addr(idx, info->wp.hw_wp_addr[sub_idx]);
		set_hw_wp_range(idx, info->wp.hw_wp_range[sub_idx]);
	}
...
static void enable_hw_watchpoint(int idx, int enable, struct perf_event *bp)
{
	...
	if (idx < 0 || idx >= K1C_HW_WATCHPOINT_COUNT)
		return;
...
{
	if (type == K1C_HW_BREAKPOINT_TYPE) {
		/* Breakpoint */
		*p = this_cpu_ptr(hbp_on_reg);
		*count = K1C_HW_BREAKPOINT_COUNT;
	} else {
		/* Watchpoint */
		*p = this_cpu_ptr(hwp_on_reg);
		*count = K1C_HW_WATCHPOINT_COUNT;
	}
}
				 ...
				 int idx, u64 ea)
{
	...
	int as = k1c_sfr_field_val(regs->es, ES, AS);
...
int check_hw_watchpoint(struct pt_regs *regs, u64 ea)
{
	...
	for (i = 0; i < K1C_HW_WATCHPOINT_COUNT; i++) {
		...
		mask = ~(BIT_ULL(info->wp.hw_wp_range[0]) - 1);
		if ((info->wp.hw_wp_addr[0] & mask) == (ea & mask)) {
			ret = 1;
			watchpoint_triggered(wp[i], regs, i, ea);
		}

		if (info->wp.use_wp1) {
			mask = ~(BIT_ULL(info->wp.hw_wp_range[1]) - 1);
			if ((info->wp.hw_wp_addr[1] & mask) == (ea & mask)) {
				ret = 1;
				watchpoint_triggered(wp[i], regs, i, ea);
			}
		}
