...
static const char *error_str[K1C_SFR_ES_ITI_WIDTH] = {
	"PSE",
	"PILSY",
	"PILDE",
	"PILPA",
...
irqreturn_t dame_irq_handler(int irq, void *dev_id)
{
	...
	unsigned long error_status = k1c_sfr_field_val(regs->es, ES, ITI);

	if (error_status) {
		pr_err("Memory Error:\n");
		for_each_set_bit(bit, &error_status, K1C_SFR_ES_ITI_WIDTH)
			pr_err("- %s\n", error_str[bit]);
	}
