...
{
	return k1c_sfr_get(K1C_SFR_PS) & (1 << K1C_SHIFT_PS_IE);
}
...
static inline notrace unsigned long arch_local_irq_save(void)
{
	...
	k1c_sfr_clear_bit(K1C_SFR_PS, K1C_SHIFT_PS_IE);
...
static inline notrace void arch_local_irq_restore(unsigned long flags)
{
	...
	if (flags)
		k1c_sfr_set_bit(K1C_SFR_PS, K1C_SHIFT_PS_IE);
	else
		k1c_sfr_clear_bit(K1C_SFR_PS, K1C_SHIFT_PS_IE);
}
...
{
	return (flags & (1 << K1C_SHIFT_PS_IE)) == 0;
}
