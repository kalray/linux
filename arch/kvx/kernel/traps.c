...
static void panic_or_kill(struct pt_regs *regs, uint64_t es, uint64_t ea,
			  int signo, int sigcode)
{
	if (user_mode(regs)) {
		do_trap(regs, signo, sigcode, ea);
		return;
	}
...
static void __name ## _trap_handler(struct pt_regs *regs, uint64_t es,	\
				    uint64_t ea)			\
{									\
	panic_or_kill(regs, es, ea, __sig, __code);			\
}
...
static void do_vsfr_fault(struct pt_regs *regs, uint64_t es, uint64_t ea)
{
	...
	panic_or_kill(regs, es, ea, SIGILL, ILL_PRVREG);
}
...
asmlinkage void do_debug(struct pt_regs *regs, unsigned long ea)
{
...
asmlinkage void do_irq(struct pt_regs *regs, unsigned long hwirq_mask)
{
