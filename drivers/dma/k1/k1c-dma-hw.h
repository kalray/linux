 ...
 * @q: Channel queue
 * @jobq: Job queue (for rx, only for eth uc. Typically, 2 must be assigned
 ...
 * @hw_id: default: -1, [0, max_desc] if assigned
 * @rx_cache_id: rx cache associated to rx job queue [0, 3]
 * @completion_count: Last hw job id completed
 */
struct k1c_dma_phy {
	struct device *dev;
	void __iomem *base;
...
/* RX queues */
int k1c_dma_noc_rx_queue_push_desc(struct k1c_dma_phy *phy,
...
/**
 * Blocking call on pending Rx descriptors
 */
...
int k1c_dma_tx_completion_try_wait(struct k1c_dma_phy *phy,
				   const u64 job_ticket);
