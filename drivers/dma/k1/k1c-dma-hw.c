...
{
	if (IS_ERR_OR_NULL(phy->q.base)) {
		dev_err(phy->dev, "RX channel hw_queue[%d] not allocated\n",
			phy->hw_id);
		return -ENOMEM;
	}
 ...
 * buffer adress to finish initialization
 */
int
...
static int k1c_dma_pkt_rx_channel_queue_init(struct k1c_dma_phy *phy)
{
	...
	if (phy->q.base == 0) {
		dev_err(phy->dev, "RX hw_queue[%d] not initialized\n",
			phy->hw_id);
		return -ENOMEM;
	}
	if (IS_ERR_OR_NULL(phy->compq.vaddr)) {
		dev_err(phy->dev, "RX comp hw_queue[%d] not allocated\n",
			phy->hw_id);
		return -ENOMEM;
	}
...
int k1c_dma_pkt_rx_job_queue_init(struct k1c_dma_phy *phy)
{
	...
	if (IS_ERR_OR_NULL(jobq))
		return -ENOMEM;

	if (IS_ERR_OR_NULL(jobq->vaddr) || IS_ERR_OR_NULL(jobq->base)) {
			       ...
			       u64 pkt_paddr, u64 pkt_len)
{
	...
	u64 ticket, read_pointer;
	int retry = K1C_DMA_READ_RETRY;
	u32 write;
	int32_t write_offset;

	ticket = k1c_dma_jobq_readq(phy, K1C_DMA_RX_JOB_Q_LOAD_INCR_WP_OFFSET);
	write = ticket & job_queue_size_mask;
	write_offset = write *
		(sizeof(struct k1c_dma_pkt_desc)/sizeof(u64));
	do {
		read_pointer = k1c_dma_jobq_readq(phy,
						  K1C_DMA_RX_JOB_Q_RP_OFFSET);
	} while (--retry && (ticket >= (read_pointer + job_fifo_size)));
...
u64 k1c_dma_rx_get_completion_count(struct k1c_dma_phy *phy)
{
	if (unlikely(phy->q.base == 0))
		return 0;
	return k1c_dma_q_readq(phy, K1C_DMA_RX_CHAN_COMP_Q_WP_OFFSET);
}

u64 k1c_dma_rx_get_inc_completion_count(struct k1c_dma_phy *phy)
{
	if (unlikely(phy->q.base == 0))
		return 0;
	return k1c_dma_q_readq(phy,
 ...
 * Blocking call
 */
int k1c_dma_rx_wait_for_completed_pkt(struct k1c_dma_phy *phy,
			     struct k1c_dma_pkt_full_desc *pkt)
{
	u64 ticket = k1c_dma_q_readq(phy,
			  K1C_DMA_RX_CHAN_COMP_Q_LOAD_INCR_RP_OFFSET);
	...
	ret = readq_poll_timeout(phy->q.base + K1C_DMA_RX_CHAN_COMP_Q_WP_OFFSET,
			   rx_comp_count,
			   ticket < (rx_comp_count + fifo_size),
			    K1C_DMA_READ_DELAY, K1C_DMA_READ_TIMEOUT);
	if (unlikely(ret)) {
		dev_err(phy->dev, "Rx completion queue full\n");
		return ret;
	}

	ret = readq_poll_timeout(phy->q.base + K1C_DMA_RX_CHAN_COMP_Q_WP_OFFSET,
				 rx_comp_count, rx_comp_count > ticket,
				 K1C_DMA_READ_DELAY, K1C_DMA_READ_TIMEOUT);
	if (unlikely(ret)) {
		dev_err(phy->dev, "Rx completion pointer %lld <= ticket %lld\n",
			rx_comp_count, ticket);
		return ret;
	}
	...
	rmb(); /* Read update */
	/* writeq to perform memory barrier */
...
int k1c_dma_tx_completion_init(struct k1c_dma_phy *phy)
{
	...
	if (IS_ERR_OR_NULL(phy->compq.base)) {
		dev_err(phy->dev, "RX job hw_queue[%d] not allocated\n",
			phy->hw_id);
		return -ENOMEM;
	}
...
u64 k1c_dma_tx_get_completion_count(struct k1c_dma_phy *phy)
{
	if (unlikely(phy->compq.base == 0))
		return 0;
	return k1c_dma_compq_readq(phy, K1C_DMA_TX_COMP_Q_WP_OFFSET);
}
					...
					u64 *hw_job_id)
{
	...
	int i, retry = K1C_DMA_READ_RETRY;
	...
	do {
		cur_read_count = k1c_dma_jobq_readq(phy,
						   K1C_DMA_TX_JOB_Q_RP_OFFSET);
	} while (--retry && (write_count >= cur_read_count + phy->max_desc));
	if (!retry) {
