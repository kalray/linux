...
#define pr_fmt(fmt)	"k1c_apic_gic: " fmt

#include <linux/irqchip/irq-k1c-apic-gic.h>
...
/**
 * struct k1c_apic_gic - k1c apic gic
...
struct k1c_apic_gic {
	...
	struct gic_in_irq_line input_irq[K1C_GIC_INPUT_IT_COUNT];
};
				...
				int enable)
{
	void __iomem *enable_line_addr = irq_line->base +
	       K1C_GIC_ENABLE_OFFSET +
	       in_irq_line->it_num * K1C_GIC_ENABLE_ELEM_SIZE;

	writeb((uint8_t) enable ? 1 : 0, enable_line_addr);
	in_irq_line->enabled = enable;
}

static void k1c_apic_gic_set_line(struct irq_data *data, int enable)
{
	struct k1c_apic_gic *gic = irq_data_get_irq_chip_data(data);
...
static void k1c_apic_gic_mask(struct irq_data *data)
{
	k1c_apic_gic_set_line(data, 0);
}

static void k1c_apic_gic_unmask(struct irq_data *data)
{
	k1c_apic_gic_set_line(data, 1);
}

#ifdef CONFIG_SMP

static int k1c_apic_gic_set_affinity(struct irq_data *d,
				     const struct cpumask *cpumask,
				     bool force)
{
	struct k1c_apic_gic *gic = irq_data_get_irq_chip_data(d);
...
static struct irq_chip k1c_apic_gic_chip = {
	.name           = "k1c apic gic",
	.irq_mask	= k1c_apic_gic_mask,
	.irq_unmask	= k1c_apic_gic_unmask,
#ifdef CONFIG_SMP
	.irq_set_affinity = k1c_apic_gic_set_affinity,
#endif
};

static int k1c_apic_gic_alloc(struct irq_domain *domain, unsigned int virq,
...
static const struct irq_domain_ops k1c_apic_gic_domain_ops = {
	.alloc  = k1c_apic_gic_alloc,
	.free   = irq_domain_free_irqs_common,
};


static void irq_line_get_status_lac(struct gic_out_irq_line *out_irq_line,
			uint64_t status[K1C_GIC_STATUS_LAC_ARRAY_SIZE])
{
	int i;

	for (i = 0; i < K1C_GIC_STATUS_LAC_ARRAY_SIZE; i++) {
		status[i] = readq(out_irq_line->base +
				  K1C_GIC_STATUS_LAC_OFFSET +
				  i * K1C_GIC_STATUS_LAC_ELEM_SIZE);
	}
}

static void k1c_apic_gic_handle_irq(struct irq_desc *desc)
{
	struct k1c_apic_gic *gic_data = irq_desc_get_handler_data(desc);
	struct gic_out_irq_line *out_line;
	uint64_t status[K1C_GIC_STATUS_LAC_ARRAY_SIZE];
	...
	for_each_set_bit(irqn, (unsigned long *) status,
			K1C_GIC_STATUS_LAC_ARRAY_SIZE * BITS_PER_LONG) {
...
static void __init
apic_gic_init(struct k1c_apic_gic *gic)
{
	...
	for (line = 0; line < K1C_GIC_INPUT_IT_COUNT; line++) {
	...
	for (cpu = 0; cpu < GIC_CPU_OUT_COUNT; cpu++) {
		...
		output_irq_line->base = gic->base +
			cpu * (K1C_GIC_ELEM_SIZE * GIC_PER_CPU_IT_COUNT);
...
static int __init
k1c_init_apic_gic(struct device_node *node, struct device_node *parent)
{
	struct k1c_apic_gic *gic;
	int ret;
	unsigned int irq;

	if (!parent) {
		pr_err("k1c apic gic does not have parent\n");
		return -EINVAL;
	}
	...
	if (of_property_read_u32(node, "kalray,intc-nr-irqs",
						&gic->input_nr_irqs))
		gic->input_nr_irqs = K1C_GIC_INPUT_IT_COUNT;

	if (WARN_ON(gic->input_nr_irqs > K1C_GIC_INPUT_IT_COUNT)) {
		ret = -EINVAL;
		goto err_kfree;
	}
	...
	irq_set_chained_handler_and_data(irq, k1c_apic_gic_handle_irq,
								gic);
...
IRQCHIP_DECLARE(k1c_apic_gic, "kalray,k1c-apic-gic", k1c_init_apic_gic);
