...
/**
 * struct kvx_irq_data - information associated to each irq
 * @ctrl:	pointer to the kvx msi controller
...
/**
 * struct kvx_msi - information for msi controller
 ...
 * @kvx_irq_data: array of data (one for each interrupt)
...
struct kvx_msi {
	...
	struct kvx_irq_data *kvx_irq_data;
	u64 msi_region;
	struct page *msi_pages;
};

/**
 * struct kvx_msi_ctrl - information for the driver
...
static u64 kvx_msi_get_addr(u64 msi_base, unsigned int bit)
{
...
static void kvx_compose_msi_msg(struct irq_data *data, struct msi_msg *msg)
{
	struct kvx_irq_data *kvx_irq_data = irq_data_get_irq_chip_data(data);
	struct kvx_msi_ctrl *ctrl = kvx_irq_data->ctrl;
	long bit = kvx_irq_data->bit;
	int num_bit = bit % IRQ_PER_ITGEN;
	u64 addr = kvx_msi_get_addr(ctrl->msi.msi_region, bit);
...
static int allocate(struct kvx_msi *msi, unsigned int nr_irqs, bool is_msi)
{
	unsigned long bit = 0;
...
static int kvx_devmsi_alloc(struct irq_domain *domain, unsigned int virq,
				unsigned int nr_irqs, void *args)
{
	struct kvx_msi_ctrl *ctrl = domain->host_data;
	struct kvx_msi *msi = &ctrl->msi;
	...
	msi_addr = kvx_msi_get_addr(msi->msi_region, bit);
	...
	for (i = 0; i < nr_irqs; i++) {
		...
		irq_domain_set_info(domain, virq + i,
				    kvx_irq_data[bit + i].parent_irq,
				    &kvx_msi_irq_chip, &kvx_irq_data[bit + i],
...
static void kvx_devmsi_free(struct irq_domain *domain, unsigned int virq,
					unsigned int nr_irqs)
{
	struct irq_data *data = irq_domain_get_irq_data(domain, virq);
	struct kvx_irq_data *kvx_irq_data = irq_data_get_irq_chip_data(data);
	struct kvx_msi *msi = &kvx_irq_data->ctrl->msi;
	long bit = kvx_irq_data->bit;
...
static const struct irq_domain_ops dev_msi_domain_ops = {
	.alloc  = kvx_devmsi_alloc,
	.free   = kvx_devmsi_free,
};

static struct irq_chip kvx_core_msi_irq_chip = {
	.name = "kvx_pcie:msi",
...
static struct msi_domain_info kvx_msi_domain_info = {
	...
	.chip = &kvx_core_msi_irq_chip,
};

static void kvx_pcimsi_uninit_irq_domain(struct kvx_msi_ctrl *ctrl,
					 int nb_itgen)
{
	struct kvx_msi *msi = &ctrl->msi;
	...
	if (msi->kvx_irq_data != NULL) {
		int i;
		unsigned int virq;

		for (i = 0; i < msi->nb_vector_max; i++) {
			virq = msi->kvx_irq_data[i].parent_irq;
			if (virq != 0)
				irq_dispose_mapping(virq);
		}
		devm_kfree(ctrl->dev, msi->kvx_irq_data);
	}
...
static int kvx_pcimsi_allocate_resources(struct kvx_msi_ctrl *ctrl)
{
	...
	msi->kvx_irq_data = devm_kzalloc(dev,
			msi->nb_vector_max * sizeof(struct kvx_irq_data),
			GFP_KERNEL);
	if (!msi->kvx_irq_data)
		return -ENOMEM;
...
static int kvx_pcimsi_connect_msi_to_itgen(struct kvx_msi_ctrl *ctrl,
					   int nb_itgen)
{
	unsigned int parent_irq;
	...
	for (i = 0; i < nb_itgen; i++) {
		...
		for (v = 0; v < IRQ_PER_ITGEN; v++) {
			...
			kvx_irq_data = &msi->kvx_irq_data[irq_num];
			irq_args.args[0] = v;
			parent_irq = irq_create_of_mapping(&irq_args);
			if (parent_irq == 0)
				return -EINVAL;
			kvx_irq_data->parent_irq = parent_irq;
			kvx_irq_data->ctrl = ctrl;
			kvx_irq_data->bit = irq_num;
			irq_set_chained_handler_and_data(parent_irq,
					 kvx_pcimsi_handler, ctrl);
			irq_num++;
		}
	}
...
static int kvx_pcimsi_config_msi_memory(struct kvx_msi_ctrl *ctrl, int nb_itgen)
{
...
static int kvx_pcimsi_init_irq_domain(struct kvx_msi_ctrl *ctrl)
{
	int nb_itgen, err;
	struct kvx_msi *msi = &ctrl->msi;
	...
	err = kvx_pcimsi_allocate_resources(ctrl);
	if (err)
		goto error;

	err = kvx_pcimsi_connect_msi_to_itgen(ctrl, nb_itgen);
	if (err)
		goto error;

	err = kvx_pcimsi_config_msi_memory(ctrl, nb_itgen);
	if (err)
		goto error;
...
error:
	kvx_pcimsi_uninit_irq_domain(ctrl, nb_itgen);
	return err;
}

static int kvx_pcimsi_parse_dt(struct kvx_msi_ctrl *ctrl,
...
static int kvx_pcimsi_device_probe(struct platform_device *pdev)
{
	struct kvx_msi_ctrl *ctrl;
	struct device *dev = &pdev->dev;
	int err;

	ctrl = devm_kzalloc(dev, sizeof(struct kvx_msi_ctrl), GFP_KERNEL);
	if (!ctrl)
		return -ENOMEM;

	ctrl->dev = dev;
	err = kvx_pcimsi_parse_dt(ctrl, pdev);
	if (err) {
		dev_err(dev, "Parsing DT failed\n");
		return err;
	}

	err = kvx_pcimsi_init_irq_domain(ctrl);
	if (err) {
