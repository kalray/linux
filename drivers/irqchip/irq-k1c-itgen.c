...
#include <linux/irqchip/irq-k1c-itgen.h>
...
/**
 * struct k1c_itgen - k1c apic mailbox
...
static void __iomem *get_itgen_cfg_offset(struct k1c_itgen *itgen,
						irq_hw_number_t hwirq)
{
	return itgen->base + K1C_ITGEN_CFG_TARGET_OFFSET +
				hwirq * K1C_ITGEN_CFG_ELEM_SIZE;
}

void __iomem *get_itgen_param_offset(struct k1c_itgen *itgen)
{
	return itgen->base + K1C_ITGEN_PARAM_OFFSET;
}

static void k1c_itgen_enable(struct irq_data *data, u32 value)
{
	struct k1c_itgen *itgen = irq_data_get_irq_chip_data(data);
	void __iomem *enable_reg =
		get_itgen_cfg_offset(itgen, irqd_to_hwirq(data)) +
		K1C_ITGEN_CFG_ENABLE_OFFSET;
...
static void k1c_itgen_mask(struct irq_data *data)
{
	k1c_itgen_enable(data, 0x0);
	irq_chip_mask_parent(data);
}

static void k1c_itgen_unmask(struct irq_data *data)
{
	k1c_itgen_enable(data, 0x1);
	irq_chip_unmask_parent(data);
}

static struct irq_chip itgen_irq_chip = {
	.name =			"k1c-itgen",
	.irq_mask =		k1c_itgen_mask,
	.irq_unmask =		k1c_itgen_unmask,
};

#define ITGEN_UNSUPPORTED_TYPES (IRQ_TYPE_LEVEL_LOW | IRQ_TYPE_EDGE_FALLING)

static int k1c_itgen_domain_alloc(struct irq_domain *domain, unsigned int virq,
...
static const struct irq_domain_ops itgen_domain_ops = {
	.alloc		= k1c_itgen_domain_alloc,
	.free		= irq_domain_free_irqs_common,
};

static void k1c_itgen_write_msg(struct msi_desc *desc, struct msi_msg *msg)
{
	struct irq_data *d = irq_get_irq_data(desc->irq);
	struct k1c_itgen *itgen = irq_data_get_irq_chip_data(d);
	uint32_t cfg_val = 0;
	...
	cfg_val |= ((dest_addr >> MB_ADDR_CLUSTER_SHIFT) &
		    K1C_ITGEN_CFG_TARGET_CLUSTER_MASK)
		   << K1C_ITGEN_CFG_TARGET_CLUSTER_SHIFT;
	cfg_val |= ((dest_addr >> MB_ADDR_MAILBOX_SHIFT) &
		     K1C_ITGEN_CFG_TARGET_MAILBOX_MASK)
		    << K1C_ITGEN_CFG_TARGET_MAILBOX_SHIFT;

	/**
	 * msg->data contains the bit number to be written and is included in
	 * the itgen config
	 */
	cfg_val |= ((msg->data << K1C_ITGEN_CFG_TARGET_SELECT_BIT_SHIFT)
		    & K1C_ITGEN_CFG_TARGET_SELECT_BIT_MASK);
...
static int
k1c_itgen_device_probe(struct platform_device *pdev)
{
	struct k1c_itgen *itgen;
	int ret;
	u32 it_count;
	...
	itgen = kzalloc(sizeof(*itgen), GFP_KERNEL);
	if (!itgen)
		return -ENOMEM;

	itgen->base = of_io_request_and_map(node, 0, node->name);
	if (!itgen->base) {
		ret = -EINVAL;
		goto err_kfree;
	}
