 ...
 * @iova	: dma address to be written as MSI address
 */
struct k1c_msi {
	struct mutex lock;
...
static void k1c_compose_msi_msg(struct irq_data *data, struct msi_msg *msg)
{
	...
	u64 addr = (u64)ctrl->msi.iova;

	/*
	 * Address and payload will be analysed by the controller in
	 * oder to determine the itgen and the bit of the itgen which
	 ...
	 */
	addr += num_itgen * ITGEN_WINDOW_SIZE;
	msg->data = num_bit & GENMASK(4, 0);
	addr |= (num_bit & GENMASK(7, 5)) << 2;
					 ...
					 int nb_itgen)
{
	...
	if (msi->msi_region != NULL)
		__free_pages(msi->msi_region, order);
}
...
static int k1c_pcimsi_config_msi_memory(struct k1c_msi_ctrl *ctrl, int nb_itgen)
{
	...
	ctrl->msi.msi_region = ptr;

	/*
	 * In order to support MSI32, always request a
	 * 32 bits address. MSI64 are supported as well
	 * MSB address will be NULL.
	 */
	err = dma_set_mask(dev, DMA_BIT_MASK(32));
	if (err)
		return err;
	msi->iova = dma_map_page(dev, ptr, 0, size, DMA_FROM_DEVICE);

	/*
	 * When an iommu is present the bus snooper will see translated address
	 * If no iommu is present then the bus snooper will see the address
	 * set in the device DMA. Due to an alias of the DDR memory to
	 * support 32 bits accesses, despite the abscence of IOMMU it cannot
	 * be assumed that "bus address" == "physical address".
	 */
	iommu_exist = false;
