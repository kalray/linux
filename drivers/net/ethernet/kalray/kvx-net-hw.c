...
void kvx_eth_lb_rfs_f_cfg(struct kvx_eth_hw *hw, struct kvx_eth_lb_rfs_f *lb_rfs)
{
	...
	if (lb_rfs->seed_command != RFS_CTRL_SEED_NO_CMD) {
		/* 0: write on seed 0, 1: write on seed 1 */
	...
	if (lb_rfs->fk_command != RFS_FK_NO_CMD) {
		...
		kvx_lbrfs_writel(hw, lb_rfs->param_ftype,
					KVX_ETH_LBR_MGMT_FLOW_TYPE_OFFSET);
		kvx_lbrfs_writel(hw, lb_rfs->param_dpatch_info,
					KVX_ETH_LBR_MGMT_DISPATCH_INFO_OFFSET);
		kvx_lbrfs_writel(hw, lb_rfs->param_flow_id,
					KVX_ETH_LBR_MGMT_FLOW_ID_OFFSET);
		kvx_lbrfs_writel(hw, (lb_rfs->fk_command << KVX_ETH_LBR_MGMT_CTRL_OPERATION_SHIFT) |
						KVX_ETH_LBR_MGMT_CTRL_RUN_MASK |
						(1 << KVX_ETH_LBR_MGMT_CTRL_DISABLE_S2F_MGMT_SHIFT),
...
static void kvx_eth_lb_rfs_f_update(void *data)
{
	...
	lb_rfs->status_tables = kvx_lbrfs_readl(hw,
		KVX_ETH_LBR_RFS_TABLES_STATUS_OFFSET);
	lb_rfs->status_wmark = kvx_lbrfs_readl(hw,
		KVX_ETH_LBR_WATERMARK_STATUS_OFFSET);
	lb_rfs->status_mgmt = kvx_lbrfs_readl(hw,
		KVX_ETH_LBR_MGMT_STATUS_OFFSET);
	lb_rfs->status_fk_part = kvx_lbrfs_readl(hw,
		KVX_ETH_LBR_MGMT_FLOW_KEY_STATUS_OFFSET + (lb_rfs->status_fk_idx<<2));
	lb_rfs->status_ftype = kvx_lbrfs_readl(hw,
		KVX_ETH_LBR_MGMT_FLOW_TYPE_STATUS_OFFSET);
	lb_rfs->status_dpatch_info = kvx_lbrfs_readl(hw,
		KVX_ETH_LBR_MGMT_DISPATCH_INFO_OFFSET);
	lb_rfs->status_flow_id = kvx_lbrfs_readl(hw,
		KVX_ETH_LBR_MGMT_FLOW_ID_STATUS_OFFSET);
	/* corruption_status part */
	lb_rfs->corr_status = kvx_lbrfs_readl(hw,
		KVX_ETH_LBR_CORRUPTION_STATUS_OFFSET);
	lb_rfs->corr_fk_part = kvx_lbrfs_readl(hw,
		KVX_ETH_LBR_CORRUPTION_FLOW_KEY_OFFSET + (lb_rfs->corr_fk_idx<<2));
	lb_rfs->corr_tables = kvx_lbrfs_readl(hw,
		KVX_ETH_LBR_CORRUPTION_TABLES_STATUS_OFFSET);
	lb_rfs->corr_fk_type = kvx_lbrfs_readl(hw,
		KVX_ETH_LBR_CORRUPTION_FLOW_TYPE_OFFSET);
}
#endif
