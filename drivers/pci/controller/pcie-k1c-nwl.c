...
struct nwl_pcie {
	...
	phys_addr_t phys_breg_base; /* Physical address Bridge Registers */
	phys_addr_t phys_csr_reg_base; /* Physical address CSR register */
	phys_addr_t phys_bar_decoder_base; /* Physical Bar decoder Base */
	phys_addr_t phys_ecam_base; /* Physical Configuration Base */
	struct regmap *ftu_regmap;
	u32 num_rc; /* index root controller (up to 8)*/
	u32 nb_lane;
	u32 breg_size;
	u32 pcie_reg_size;
	u32 ecam_size;
			     ...
			     struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(pdev->dev.of_node, "nb_lane",
				   &pcie->nb_lane);
	if (ret != 0)
		return ret;
	pr_info("nb_lane : %u\n", pcie->nb_lane);
